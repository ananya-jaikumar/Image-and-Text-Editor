...
def GUI():
    ...
    class ImageButcher(Tk):
        def __init__(self):
            ...
            currdir = os.getcwd()
            self.path = tkinter.filedialog.askopenfilename(parent=root, initialdir=currdir, title='Select an image')
...
def BATCH_PROCESSING():
    ...
    root.lift()
    root.focus_force()
    path = tkinter.filedialog.askdirectory(parent=root) # Or some other dialog
    root.destroy()
    ...
    while True:
        option=input('Enter the desired option(1-6): ')
        if option=='q':break
        try:
            option=int(option)
            ...
            continue
        if option==2:size=list(map(int,(input('Enter a,b where (a x b) is the desired size of image:')).split(',')))
        for imagepath in images:
            ...
            im.save(location)
        control=input('The desired option has been carried out successfully. Do you want to continue?(y/n): ')
        if control=='n':break
...
while True:
    option=input('Enter the desired option(1/2): ')
    if option=='1': GUI()
